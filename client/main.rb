...
numbers = []
  ...
  begin
    response = JSON.parse(HttpConnection.get('/number'))
    numbers << response["number"]
  end until response["stop_asking"]

  the_sum = numbers.inject(:+)

  response = HttpConnection.post('/sum', body: { the_sum: the_sum } )
  puts JSON.parse(response)['message']
